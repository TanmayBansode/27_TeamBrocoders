...
export default function EnhancedUserDashboard() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        console.log({ getCodeSnippetsFiles });
        console.log({ getCodeSnippetsFiles });
        console.log({ getCodeSnippetsFiles });
        console.log({ getCodeSnippetsFiles });
        console.log({ getCodeSnippetsFiles });
        ...
        const codeSnippets = await Promise.all(
            getCodeSnippetsFiles.map(async (file: string, i: Number) => {
                ...
                const raw = JSON.stringify({
                    file_slug:
                        "Scalable-Notification-System/internal/rabbitmq/scheduler.go",
                    query: "Worker Pool",
                });
        ...
        // getCodeSnippetsFiles.map(async (file: string) => {
        //     const explainCodeSnippet = await fetch(
        //         "http://127.0.0.1:5000/explain-code",
        //         {
        //             method: "POST",
        //             headers: {
        //                 "Content-Type": "application/json",
        //             },
        //             body: JSON.stringify({ file_slug: file }),
        //         }
        //     );
        // });
        const explainCodeSnippet = await Promise.all(
            getCodeSnippetsFiles.map(async (file: string) => {
                const explainCodeSnippet = await fetch(
                    "http://127.0.0.1:5000/explain-code",
                    {
                ...
                return explainCodeSnippet;
            })
        );

        const explainCodeSnippetData = await Promise.all(
            explainCodeSnippet.map(async (explainCodeSnippet) =>
                explainCodeSnippet.json()
            )
        );
        ...
        setDocumentationTexts(
            explainCodeSnippetData.map((data) => data.explanation)
        );
        console.log({ documentationTexts });
                                ...
                                <Card>
                                    ...
                                    <CardContent>
                                        <p className="text-sm text-gray-600 dark:text-gray-300 mb-4">
                                            ...
                                            using Code 2 as it provides better
                                            performance and follows the
                                            project's coding standards more
                                            closely.
                                        </p>
